...
def reduce_L1_Bin (data, dark, threshold, max_frames=10):
    """
    ...
    for frame_index in range(max_frames):
        ...
        sm = coo_matrix((pixvals, (y1,x1)), shape=(data.shape[1], data.shape[2]))
...
def reduce_L1 (data, dark, threshold, max_frames=10):
    ...
    feature_dict = {}
    s = nd.generate_binary_structure(2,2)
    reduced_frames = np.zeros((n_Frames, data.shape[1], data.shape[2]), dtype=np.uint16)
    for frame_index in range(max_frames):
        dark_subtracted = (data[frame_index]).astype(np.int32) - _thresh_frame.astype(np.int32)
        print(data[frame_index][0,0])
        print(dark[0,0])
        print(dark_subtracted[0,0])
        _binary_ind = dark_subtracted > 0
        (y1,x1) = np.nonzero(_binary_ind)
        pixvals = dark_subtracted[_binary_ind]
        reduced_frames[frame_index][_binary_ind] = pixvals
