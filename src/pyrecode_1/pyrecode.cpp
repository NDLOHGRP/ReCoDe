...
PyObject *
_bit_pack_pixel_intensities (RecodeReader *self, PyObject* args) {
	...
	printf("current position (pyrecode.cpp): %d\n", ftell(self->file));
	*/
	float t = bit_pack_pixel_intensities (sz_packedPixval, n_fg_pixels, bit_depth, (uint16_t *)(&view_pixvals)->buf, (uint8_t *)(&view_packed_pixvals)->buf);
	//printf("Decoded Frame with %d foreground pixels\n", n);
...
PyMethodDef ReCoDeMethods[] = 
{
	...
	{ "_bit_pack_pixel_intensities", (PyCFunction)_bit_pack_pixel_intensities, METH_VARARGS, 0 },
	{0,0,0,0}
};
