...
def _get_pixel_thresh_2 (d, nx, ny, expected_n_events, t):
    ...
    for r in numba.prange(nx):
        for c in range(ny):
            ...
            top_values = np.ones((expected_n_events+1), dtype=np.float32)*np.finfo(np.float32).min      # replace with np.finfo(np.float32).min
...
def make_calibration_frames (filepath, dtype, nFrames, n_stats_frames, n_sigmas, savepath='', filename_prefix='', use_acc=False, sigma_acc=-1):
...
if __name__== "__main__":
    ...
    parser.add_argument('--sigma_acc', dest='sigma_acc', action='store', type=int, default=3, help='which sigma to use for accurate calibration when use_acc=True')
