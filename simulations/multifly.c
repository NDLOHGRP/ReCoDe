/**************************************************************************************************************
to compile:    gcc multifly.c -lz -lbz2 -llzma -lsnappy -llz4 -O3 -o multifly -lm -fopenmp
to compile with profiling: kinst-ompp-papi gcc multifly.c -lz -O3 -o multifly -lm -fopenmp
...
void runMTT (int NT, unsigned long **RCT_Buffer_Fill_Positions, unsigned long MAX_POLLING_ITERS, int POLLING_TIME_STEP) {
    ...
    recode_print("MMT: Tracking Started.\n");
    ...
    recode_print("MMT: Tracking Stopped.\n");
}

// off-loader thread
void runOLT (int NT, 
			 uint8_t *Connected_OLTs,
			 uint8_t process_id,
    ...
    int FDs[NT-1];
    for (i = 0; i < NT; i++) {
		if (Connected_OLTs[i] == process_id) {
            ...
            partFiles[i] = fopen(concat("/gpfs0/scratch/loh/abhik/code_data/DataCompression/on_the_fly_output/", filename), "wb");
			FDs[i] = fileno(partFiles[i]);
			fwrite (&i, sizeof(char), 1, partFiles[i]);        // write process_id to partfile
		}
    }

    int num_open_threads = 0;
	for (i = 0; i < NT; i++) {
		if (Connected_OLTs[i] == process_id) {
			num_open_threads++;
		}
	}
	recode_print("OLT %d: Ready.\n", process_id);
    ...
    while (1) {
        ...
        for (i = 0; i < NT; i++) {
			...
			if (Connected_OLTs[i] == process_id) {
				...
				if ((*RCT_FULL_INDs)[i] == 1) {                // RCT indicates buffer is full, off load data to file
					recode_print("OLT %d: RCT %d's buffer is full, trying to clear %u bytes to disk.\n", process_id, i, (*RCT_Buffer_Fill_Positions)[i]);
					...
					if (rc_write_mode > RC_WRITE_MODE_RETURN_TO_OLT) {
						...
						fdatasync(FDs[i]);
						//recode_print("Writing to disk.\n");
					}
					
					(*RCT_FULL_INDs)[i] = 2;                    // indicate RCT buffer has been off-loaded
					recode_print("OLT %d: RCT %d is cleared as requested.\n", process_id, i);
				}

				if ((*RCT_FULL_INDs)[i] == 3) {                // RCT indicates it is done, close RCT's file
					
					
					recode_print("OLT %d: RCT %d indicates it is done. Flushing available buffer data to file.\n", process_id, i);
					...
					recode_print("OLT %d: RCT %d is closed.\n", process_id, i);
				}
        ...
        if (num_open_threads == 0) {                    // All RCTs are done, exit
            ...
            ALL_RCT_DONE = 1;
            recode_print("OLT %d: All RCTs are done.\n", process_id);
            return;
        }
...
void recode_on_the_fly (int level, 
    ...
    */
    ...
    if (doMemTracking == 0) {
        ...
        #pragma omp parallel for num_threads(NT_RCT + NT_OLT + 1)
        for (i = 0; i < (NT_RCT + NT_OLT + 1); i++) {

            if (i == 0) {
                
				runMTT (NT_RCT, &RCT_Buffer_Fill_Positions, 80, 250);
...
int main(int argc, char *argv[]) {
    ...
    for (dr = 0; dr < 4; dr++) {
        ...
        */
        ...
        b.nz = nImageFrames;
        recode_print("Image Data Replicated %d times.\n", replicates);
        */
        ...
        */
        ...
        recode_print("Dark data loaded.\n");
        getDarkMax (darkBuffer, d, darkFrame, 25);
        free(darkBuffer);
        recode_print("Dark noise estimation complete.\n");
        */
        ...
        for (threads = 5; threads <=20; threads = threads + 5) {
            for (mem = 10; mem <= 50; mem = mem + 10) {
                recode_print("Threads = %d, Memory = %d MB\n", threads, mem);
        ...
        */
        ...
        fprintf(logfile, "\nRun Start Time: %s\n", buffer);
        fprintf(logfile, "Data Not Saved (Compression Only Time)\n");
        fprintf(logfile, "=======================================\n");
        ...
        uint8_t rct_threads = 0;
		uint8_t olt_threads = 2;
        
        for (level = 1; level < 2; level += 3) {
            for (rct_threads = 15; rct_threads < 41; rct_threads = rct_threads + 5) { 
				for (olt_threads = 1; olt_threads < 2; olt_threads++) { 
